...
The `FunctionalTestRunner` automatically transpiles functional tests using babel, so that tests can use the same ECMAScript features that {kib} source code uses. Refer to {kib-repo}/tree/{branch}/STYLEGUIDE.md#typescriptjavascript[STYLEGUIDE.md].
