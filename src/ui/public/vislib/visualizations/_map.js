...
export default function MapFactory(Private, config) {

  let defaultMapZoom = 2;
  let defaultMapCenter = [15, 5];
  let defaultMarkerType = 'Scaled Circle Markers';

  var mapConfig = config.get('visualization:tileMap:leafletDefaults');
  let mapTiles = {
    url: mapConfig.url,
    options: {
      attribution: marked(mapConfig.options.attribution),
      subdomains: mapConfig.options.subdomains
    }
  };
  ...
  function TileMapMap(container, chartData, params) {
    ...
    let mapOptions = {
      minZoom: mapConfig.options.minZoom,
      maxZoom: mapConfig.options.maxZoom,
      noWrap: true,
